...
Hunch needs the **Accessibility** permission — the same grant a keylogger would ask for — to read the focused text field, catch the accept key, and type suggestions back. Nothing you type is ever uploaded.

Don't take that on faith. The entire input path is three files worth reading: [`AXText.swift`](../Sources/Hunch/AXText.swift) reads the field, [`KeyTap.swift`](../Sources/Hunch/KeyTap.swift) watches for the accept key, [`TextInjector.swift`](../Sources/Hunch/TextInjector.swift) types back.
...
| **Accessibility** *(required)* | How Hunch reads the focused field, catches the accept key, and types text back. There is no way to do this without it. |
...
| **Stored locally** | A journal of suggestions and short snippets of surrounding text in `~/Library/Application Support/Hunch`, capped at 50 MB, used for on-device personalization — plus any text you import yourself via **Settings → Personalization → Import Text…**. Turning the journal off deletes all of it, imported passages included. |
...
Hunch keeps everything in four places — remove them all and no trace remains:
...
rm -rf /Applications/Hunch.app

# 2. Downloaded models (several GB). Settings → Model lists each one with its
#    size and a Delete button, which only ever touches Hunch's own catalog —
...
rm -rf ~/Library/Application\ Support/Hunch

# 4. Settings and the permission grants (drop the last two if you never
#    turned on screen context or dictation)
defaults delete app.hunch.Hunch
tccutil reset Accessibility app.hunch.Hunch
tccutil reset ScreenCapture app.hunch.Hunch
tccutil reset Microphone app.hunch.Hunch
```

Installed with Homebrew? `brew uninstall hunch` replaces step 1; steps 2–4 still apply.
